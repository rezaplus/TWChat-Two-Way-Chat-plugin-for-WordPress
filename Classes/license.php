...
class TWCH_license
{
    ...
    public static function isValid_S(){
        $key = get_option('TWCH_license')['key'];
        $result = '';
        if(!empty($key)){
    ...
    public static function Activate_DL($licenseKey){
        update_option('TWCH_license',array('status'=>'active','key'=>$licenseKey));
        if(!empty(get_option('gen_setgs_temp_d2w2p'))){
            update_option('TWCH_General_Option',get_option('gen_setgs_temp_d2w2p'));
        }else{
            update_option('TWCH_General_Option',Array
                (
                    'float_is_enable' => '',
                    'Applicationmode' => 'app' ,
                    'fix_countrycode' => '',
    ...
    public static function Deactive_DL(){
        update_option('TWCH_license',array('status'=>'deactive','key'=>''));
        update_option('gen_setgs_temp_d2w2p',get_option('TWCH_General_Option'));
        update_option('TWCH_General_Option',Array
            (
                'float_is_enable' => '',
                'Applicationmode' => 'app' ,
